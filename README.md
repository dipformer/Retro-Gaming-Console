...
- `printNumberToMatrix (uint8_t n, uint8_t x, uint8_t y)` - выводит значение цифры `n` oт 0 до 9 в позицию `(x, y)` массива `GameBoard.matrix[0..15]`.
- `startEffect (uint8_t effect)` - запускает эффект `effect` при выводе следующего кадра. Доступны эффекты `GB_MATRIX_EFFECT_LINES`, `GB_MATRIX_EFFECT_LEFT`, `GB_MATRIX_EFFECT_RIGHT`. Функция устанавливает, что при следующем вызове `GameBoard.show()` необходимо не просто отобразить данные, а отобразить их с соответствующим эффектом.
